...
`.mcp.json` registers the NASA Earthdata MCP server (github.com/nasa/earthdata-mcp), a streamable-http server over NASA's Common Metadata Repository. Entry verified against the repository README on 2026-07-04 (Session 1; per the implementation guide, entries are verified at authoring time, never trusted from earlier drafts).
...
**Graceful degradation:** discover-data does not require this connector. When the server is unreachable it falls back to knowledge-based discovery with archive URLs (SPECIFICATION.md v0.5.1 §3.4) and names the fallback; loading proceeds from local files or direct library access.
