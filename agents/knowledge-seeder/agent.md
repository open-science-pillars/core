...
description: "Draft OKF knowledge concepts (dataset + gotcha candidates) from steward-supplied authoritative sources: every claim cites a source entry, status draft, never merged. Crawls only supplied domains."
...
2. **Draft one dataset concept** conforming to SPEC §5.2: type,
   ...
   (today, since you looked), a `sources:` list (one entry per page
   used, each with `id`, `resource` and `title`), an `## Uncertainty`
   section from what the sources actually say about errors and
   quality, and `status: draft`. Never write the legacy `timestamp`,
   `evidence` or `verified_by` keys; the linter flags them as an
...
3. **Draft gotcha candidates**, one file per trap, for every
   ...
   `status: draft`, a `generated` event, and at least one `sources`
   entry cited from the body per claim.
4. **Every claim is paraphrased WITH a citation** (`[^id]` footnote
   naming a `sources` entry) to the page that supports it, so the
   bundle checker can join each claim to its source. An unclear or
...
- Never merge, never edit index.md or log.md, never set status beyond
  draft, never add a `verified` event (steward approval adds it).
- Never crawl beyond the supplied domains.
- Never assert without a cited source; never fabricate URLs.
